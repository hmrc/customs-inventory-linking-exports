...
trait DynamicServiceTestData {
  ...
  protected val noContextService = "no-context-service"
  protected val noContextServiceHost = "no-context-host"
  protected val noContextServicePort = 2222
  protected val noContextServiceBearer = "no-context-bearer"
  ...
  val configuredApplication = GuiceApplicationBuilder(
    ...
    configure(
      Map(
        ...
        s"microservice.services.$noContextService.host" -> noContextServiceHost,
        s"microservice.services.$noContextService.port" -> noContextServicePort,
        s"microservice.services.$noContextService.bearer-token" -> noContextServiceBearer,
      )).build()
}
