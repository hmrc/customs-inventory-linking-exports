...
import uk.gov.hmrc.customs.inventorylinking.export.config.{InvalidEnvironmentException, ServiceConfigProvider}
...
class ServiceConfigProviderSpec extends IntegrationTestSpec with GuiceOneAppPerSuite with DynamicServiceTestData {
  ...
  "getting service configuration" when {
    ...
    "environment has a service with no bearer token" should {
      "bearer token is None" in {
        val res = serviceConfigProvider.getConfig(noBearerService)
        res.bearerToken shouldBe None
      }
    }

    "environment has a service with no context" should {
      "throw an InvalidEnvironmentException" in {
        val thrown = intercept[InvalidEnvironmentException] {
          serviceConfigProvider.getConfig(noContextService)
        }

        thrown.getMessage shouldBe s"Context missing for service $noContextService in default environment"
      }
    }
  }
}
