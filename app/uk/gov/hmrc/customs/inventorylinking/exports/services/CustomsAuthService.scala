...
import uk.gov.hmrc.customs.inventorylinking.exports.controllers.ErrorResponse.{ErrorInternalServerError, UnauthorizedCode}
...
class CustomsAuthService @Inject()(override val authConnector: AuthConnector,
  ...
  private val customsInventoryLinkingexportsEnrolment = "write:customs-inventory-linking-exports"
  ...
  */
  def authAsCsp(implicit vhr: HasConversationId, hc: HeaderCarrier): Future[Either[ErrorResponse, IsCsp]] = {
    val eventualAuth: Future[Either[ErrorResponse, IsCsp]] =
        authorised(Enrolment(customsInventoryLinkingexportsEnrolment) and AuthProviders(PrivilegedApplication)) {
          Future.successful[Either[ErrorResponse, IsCsp]] {
            logger.debug(s"Successfully authorised CSP PrivilegedApplication with $customsInventoryLinkingexportsEnrolment enrolment")
            Right(true)
          }
        }
    ...
    eventualAuth.recover{
      case NonFatal(ae: AuthorisationException) =>
        logger.debug(s"No authorisation for CSP PrivilegedApplication with $customsInventoryLinkingexportsEnrolment enrolment", ae)
        Right(false)
      case NonFatal(e) =>
        logger.error(s"Error when authorising for CSP PrivilegedApplication with $customsInventoryLinkingexportsEnrolment enrolment", e)
